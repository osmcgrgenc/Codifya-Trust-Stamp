...
const StatsCards = dynamic(() => import('./components/StatsCards').then(mod => ({ default: mod.StatsCards })), {
  loading: () => <div className="grid md:grid-cols-3 gap-6 mb-8">
    {[1, 2, 3].map(i => <div key={i} className="h-32 bg-gray-100 animate-pulse rounded" />)}
  </div>
})

const TestimonialsList = dynamic(() => import('./components/TestimonialsList').then(mod => ({ default: mod.TestimonialsList })), {
  loading: () => <div className="space-y-4">
    {[1, 2, 3].map(i => <div key={i} className="h-24 bg-gray-100 animate-pulse rounded" />)}
  </div>
})

const ShareLink = dynamic(() => import('./components/ShareLink').then(mod => ({ default: mod.ShareLink })), {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Suspense fallback={<div className="grid md:grid-cols-3 gap-6 mb-8">
          {[1, 2, 3].map(i => <div key={i} className="h-32 bg-gray-100 animate-pulse rounded" />)}
        </div>}>
